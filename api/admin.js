...
app.post('/api/adb/sync/orders', async (_req,res)=>{
  try{
    const r = await fetch(API_BASE.replace(/\/$/,'') + '/api/orders?includeCleared=1');
...
// Update order status / clear flags
app.patch('/api/adb/orders/:id', auth, async (req,res)=>{
  try{
    ...
    // forward to order API (optional)
    await fetch(API_BASE.replace(/\/$/,'') + '/api/orders/'+encodeURIComponent(id), { method:'PATCH', headers:{ 'Content-Type':'application/json', 'Authorization':'Bearer '+ADMIN_PASSWORD }, body: JSON.stringify({ status, cleared }) });
    res.json({ ok:true });
  }catch(e){ res.status(500).json({ ok:false }); }
});

// Refund via Toss (proxy to order API for simplicity)
app.post('/api/adb/refund', auth, async (req,res)=>{
  try{
    ...
    const r = await fetch(API_BASE.replace(/\/$/,'') + '/api/toss/refund', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ paymentKey, cancelAmount: amount, cancelReason: reason }) });
...
app.get('/api/adb/menu', async (_req,res)=>{
  try{
    const r = await fetch(API_BASE.replace(/\/$/,'') + '/api/menu');
    if(!r.ok) return res.status(502).send('api fail');
    const arr = await r.json();
    for(const m of arr){
      await withClient(c=> c.query(`
...
// Daily code cache (pull and store)
app.get('/api/adb/daily-code', async (_req,res)=>{
  try{
    const r = await fetch(API_BASE.replace(/\/$/,'') + '/api/daily-code');
    if(!r.ok) return res.status(502).send('api fail');
...
// Sales stats (daily/weekly/monthly/yearly)

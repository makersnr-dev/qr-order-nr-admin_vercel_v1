...
export default async function handler(req, res) {
  try {
    ...
    let raw=''; await new Promise((resolve)=>{ req.on('data', c=> raw+=c); req.on('end', resolve); });
    if (!raw) raw='{}';
    ...
    const opts = { method:'POST', headers:{ 'Content-Type':'application/json', 'Content-Length': Buffer.byteLength(raw) } };
    ...
    upstream.on('error', (e) => { res.status(500).json({ ok:false, error:String(e) }); });
    upstream.end(raw);
  } catch (e) {
